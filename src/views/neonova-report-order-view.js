class NeonovaReportOrderView {
    constructor(username, friendlyName) {
        ...
        this.tab = null;
        this.onGenerate = null;  // callback for real generation: (startDate) => {}
    }
                ...
                <script>
                    ...
                    document.querySelectorAll('.quick-btn').forEach(btn => {
                        btn.addEventListener('click', () => {
                            ...
                            window.parent.generateReport(start);  // call parent controller
                        });
                    });
                    ...
                    document.getElementById('generate-custom').addEventListener('click', () => {
                        ...
                        window.parent.generateReport(start);
                    });

                    // Placeholder - real generation will be called from parent
                    window.generateReport = function(startDate) {
                        ...
                        const interval = setInterval(() => {
                            ...
                            document.getElementById('status').textContent = pct < 100 ? 'Generating...' : 'Complete!';
                            if (pct === 100) {
                                clearInterval(interval);
                                document.getElementById('progress').style.display = 'none';
                                document.getElementById('report-content').innerHTML = '<h2>Report Ready</h2><p>Full report content will appear here.</p>';
                            }
                        }, 300);
                    };
                </script>
    ...
    showProgress() {
        this.tab.document.querySelector('.quick-buttons').style.display = 'none';
        this.tab.document.querySelector('.custom-range').style.display = 'none';
        this.tab.document.getElementById('progress').style.display = 'block';
    }
    ...
    showReport(reportHTML) {
        if (this.tab && !this.tab.closed) {
            const progress = this.tab.document.getElementById('progress');
            const status = this.tab.document.getElementById('status');
