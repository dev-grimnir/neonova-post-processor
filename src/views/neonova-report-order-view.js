/**
 * Report order modal — allows user to select date range and generate report.
 */
class NeonovaReportOrderView extends BaseNeonovaView {
    /**
     * @param {HTMLElement|null} container - Optional container (used when embedded)
    ...
    constructor(container, username, friendlyName) {
        ...
        this.onGenerateRequested = null;  // Callback for when user clicks generate
    }
        ...
        this.onGenerateRequested = (startIso, endIso) => {
            ...
            setTimeout(() => {
                ...
                this.controller.generateReportData(
                    ...
                    new Date(startIso),
                    new Date(endIso),
                    (collected, total, page) => {
                        progressView.updateProgress(collected, total, page);
                    }
                ).then(data => {
    ...
    /**
     * Renders the form content (quick presets + custom date picker).
     */
    #renderContent() {
    ...
    /**
     * Populates year/month/day dropdowns with sensible defaults.
     */
    #populateDatePickers() {
        ...
        // Default: start = 1 year ago, end = today
        ...
        populateYears(sy, oneYearAgo.getFullYear());
        populateMonths(sm, oneYearAgo.getMonth() + 1);
        populateDays(sd, oneYearAgo.getFullYear(), oneYearAgo.getMonth() + 1, oneYearAgo.getDate());
        ...
        populateYears(ey, currentYear);
        populateMonths(em, currentMonth);
        populateDays(ed, currentYear, currentMonth, currentDay);
