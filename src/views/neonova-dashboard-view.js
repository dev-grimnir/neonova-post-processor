    ...
    render() {
        ...
        if (slider && display) {
            ...
            slider.addEventListener('touchmove', (e) => {    // optional mobile support
                ...
                // ────── SINGLE EVENT DELEGATION (survives every render) ──────
            this.panel.onclick = (e) => {
                const t = e.target.closest('button');   // catches buttons even if wrapped
                if (!t) return;
                ...
                // Refresh Now
                if (t.classList.contains('refresh-btn')) {
                    this.controller.poll();
                }
    
                // Minimize button in header
                if (t.classList.contains('minimize-btn')) {
                    this.toggleMinimize();
                }
            ...
            // Friendly name editing (still needs per-cell listeners)
            this.panel.querySelectorAll('.friendly-name').forEach(cell => {
                ...
                cell.addEventListener('click', () => {
                    ...
                    // ... rest of your input + save logic exactly as before ...
                });
            });
        }
    
        // ← NEW HELPER METHOD — keeps render() clean
        openReportModal(username, friendlyName) {
            // Paste your entire original report modal code here
            // (the overlay, NeonovaReportOrderView, progressView, generateReportData, etc.)
            // Exactly the same as you had before — just move it out of render()
        }
    }
