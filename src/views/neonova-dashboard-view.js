...
class NeonovaDashboardView extends BaseNeonovaView {
    ...
    /**
     * Renders the fixed header with title and minimize button.
     */
    #renderHeader() {
        const headerHTML = `
        ...
        // Insert at top (replace if exists or append)
        let header = this.panel.querySelector('.flex-col > div:first-child');
        if (header) {
            header.outerHTML = headerHTML;
        } else {
            this.panel.insertAdjacentHTML('afterbegin', `<div class="flex flex-col h-full">${headerHTML}</div>`);
        }
    }

    /**
     * Renders the add customer bar (fixed, never scrolls).
     */
    #renderAddBar() {
        const addBarHTML = `
    ...
    /**
     * Renders the scrollable customer table.
     */
    #renderCustomerTable() {
        let rows = '';
            ...
            <div class="flex-1 overflow-y-auto px-8 pb-8 neonova-scroll">
                <div class="bg-zinc-900 border border-zinc-700 rounded-3xl overflow-hidden">
                    <table class="w-full">
                        ...
                        <tbody>${rows || '<tr><td colspan="5" class="p-12 text-center text-zinc-500">No customers added yet</td></tr>'}</tbody>
                    </table>
                </div>
            </div>
        `;
    ...
    /**
     * Renders the fixed bottom bar with polling controls.
     */
    #renderBottomBar() {
        const bottomBarHTML = `
     ...
     * Called once per render to ensure fresh bindings.
     */
    #attachEventListeners() {
        // Remove buttons
        this.panel.querySelectorAll('.remove-btn').forEach(btn => {
            btn.replaceWith(btn.cloneNode(true)); // Remove old listeners
            this.panel.querySelectorAll('.remove-btn').forEach(b => {
        ...
        if (pollBtn) {
            pollBtn.replaceWith(pollBtn.cloneNode(true));
    ...
    /**
     * Sets up the polling interval slider and tooltip.
     */
    #setupPollingSlider() {
        ...
        // Tooltip logic (unchanged but isolated)
    ...
    /**
     * Sets up inline editing for friendly names.
     */
    ...
    /**
     * Handles minimize/maximize animation and state.
     */
    toggleMinimize() {
